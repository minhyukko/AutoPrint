...
//		combo.setItems ("Write your file directory location here:", "C:");
		...
		ArrayList<TreeItem> filenameList = new ArrayList<>();
		
        // Create a label to display the selected text
//        Label label = new Label(shell, SWT.NONE);
//        label.setText("Selected item: ");
//        label.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 2, 1));
        Button checkAllButton = new Button(shell, SWT.PUSH);
...
//        });
        ...
        checkAllButton.addListener(SWT.Selection, event -> {
            TreeItem[] items = tree.getItems();
            for (TreeItem item : items) {
                item.setChecked(true);
                TreeItem[] variableItems = item.getItems();
                for (TreeItem variableItem : variableItems) {
                	variableItem.setChecked(true);
                }
            }
        });
...
//   				TreeItem checkAll = new TreeItem(tree, SWT.NONE);
//   				checkAll.setText("CHECK ALL");
//   				ti.add(checkAll);
                // Create a button to check all checkboxes
...
//		new Label(shell, SWT.NONE).setText(combo.getText());
		...
		rmAll.setText("Remove Checked Print Statements");
		
		Button none = new Button(shell, SWT.RADIO);
		none.setText("Insert Print Statements");
...
//		                    long startTime = System.currentTimeMillis();
		                    removePrintStatementsFromFile(combo.getText() + "\\" + ti.get(i).getText().toString(), varList);
//		                    long endTime = System.currentTimeMillis();
//		                    long executionTime = endTime - startTime;
//		                    longList.add(executionTime);
//		                    filenameList.add(ti.get(i));
...
//        }
    ...
    public static void removePrintStatementsFromFile(String filePath, ArrayList<String> variables) {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
            	...
            	if (variables.size() > 0) { // Removes variables checked by the user if any variable is checked.
            		for (int i = 0; i < variables.size(); i++) {
            			if (line.contains(variables.get(i)) && line.contains("System.out.print")) {
            				hasVariable = true;
            			}
            		}
            		if (line.contains("System.out.print") && hasVariable) {
